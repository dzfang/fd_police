...
public class CXSupplierCombo extends TagBase {
	/**
	...
	public void startTag(StringBuilder sb) {
		...
		sb.append("<input type='hidden' id='comboSupplierCode_" + id + "' />");
		sb.append("<input type='hidden' id='comboSupplierName_" + id + "' />");
		sb.append("<input type='hidden' id='comboSupplierText_" + id
				+ "'  value='" + (text == null ? "" : text) + "'/>");
		// 通过js初始化combogrid
		sb.append("<script>");
		sb.append("$(function(){");
		sb.append("combogridSupplier('" + id + "'," + multiple + ", "
				+ panelWidth + ");");
		sb.append("		});");
		sb.append("</script>");
