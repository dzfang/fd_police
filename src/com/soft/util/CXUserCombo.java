...
public class CXUserCombo  extends TagBase {
	/**
	...
	public void startTag(StringBuilder sb) {
		...
		sb.append("<input type='hidden' id='comboUserCode_" + id + "' />");
		sb.append("<input type='hidden' id='comboUserName_" + id + "' />");
		sb.append("<input type='hidden' id='comboUserText_" + id
				+ "'  value='" + (text == null ? "" : text) + "'/>");
		// 通过js初始化combogrid
		sb.append("<script>");
		sb.append("$(function(){");
		sb.append("combogridUser('" + id + "', " + multiple + ", "
				+ panelWidth + ");");
		sb.append("		});");
		sb.append("</script>");
	}
